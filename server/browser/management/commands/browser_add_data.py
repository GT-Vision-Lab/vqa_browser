...
# Good if you know database is empty,
# i.e., aren't rerunning after a failure.
FORCE = True
...
#                  }
    ...
    def add_images(self, all_data, dataset, subset,
        ...
        for data in all_data['images']:
            ...
            if FORCE:
                ...
                img_obj.save()
            elif not Image.objects \
                        .filter(image_id=img['image_id']).exists():
                img_obj = Image(image_name=img['image_name'],
                                image_id=img['image_id'],
                                subset=img['subset'],
                                url_base=img['url_base']
    ...
    def add_obj_annotations(self, all_data, dataset, subset,
        ...
        for data in all_data['categories']:
            if not Category.objects.filter(cat_id=data['id']).exists():
        ...
        if dataset == 'mscoco':
            for idx, data in enumerate(all_data['annotations']):
                if FORCE:
                    ...
                    ann_obj.save()
                elif not Annotation.objects \
                          .filter(ann_id=data['id']).exists():
                    img_obj = Image.objects.get(image_id=data['image_id'])
                    ann_obj = Annotation(image=img_obj,
                                         ann_id=data['id'],
                                         cat_id=data['category_id'],
    ...
    def calc_obj_ann_counts(self, dataset, subset,
        ...
        for datum in data:
            if FORCE: 
                ...
                ann_count.save()
            elif not AnnotationCount.objects \
    ...
    def add_captions(self, all_data, dataset, subset,
        ...
        for data in all_data['annotations']:
            if FORCE:
                ...
                ann_obj.save()
            elif not Caption.objects.filter(cap_id=data['id']).exists():
                img_obj = Image.objects.get(image_id=data['image_id'])
                ann_obj = Caption(image=img_obj,
                                  cap_id=data['id'],
                                  caption=data['caption'],
            ...
            for img_key in img_data:
                if img_key in [str(idx) for idx in range(1, 11)]:
                    ...
                    if FORCE:
                        ...
                        q.save()
                        self.add_answers(Answer, ques_data, img_obj, q)
                    elif not Question.objects \
                        .filter(ques_id=data['quesID']).exists():
                        q = Question(
                            image=img_obj,
                            ques_id=ques_data['quesID'],
                            question=ques_data['quesStr'].encode('utf-8'),
    ...
    @transaction.atomic
    def add_answers(self, Answer, ques_data, img_obj, q):
